...
/// The domain separator for traceable anonymous-to-transparent, for the Plonk proof.
const TABAR_TO_AR_PLONK_PROOF_TRANSCRIPT: &[u8] = b"TABAR to AR Plonk Proof";

/// The domain separator for traceable anonymous-to-transparent, for address folding.
const TABAR_TO_AR_FOLDING_PROOF_TRANSCRIPT: &[u8] = b"TABAR to AR Folding Proof";
...
/// Generate a traceable anonymous-to-transparent pre-note.
pub fn init_t_abar_to_ar_note<R: CryptoRng + RngCore>(
    prng: &mut R,
    oabar: &OpenAnonAssetRecord,
    abar_keypair: &KeyPair,
    ar_pub_key: &PublicKey,
) -> Result<TAbarToArPreNote> {
    ...
    let body = TAbarToArBody {
        ...
        memo: owner_memo,
        auditor_memos: TAxfrAuditorMemo::default(), //
    };
...
/// Finalize a traceable anonymous-to-transparent note.
pub fn finish_t_abar_to_ar_note<R: CryptoRng + RngCore, D: Digest<OutputSize = U64> + Default>(
    prng: &mut R,
    params: &ProverParams,
    pre_note: TAbarToArPreNote,
    hash: D,
) -> Result<TAbarToArNote> {
    let TAbarToArPreNote {
        body,
        witness,
...
/// Verify the traceable anonymous-to-transparent note.
pub fn verify_t_abar_to_ar_note<D: Digest<OutputSize = U64> + Default>(
    ...
    let mut transcript = Transcript::new(TABAR_TO_AR_FOLDING_PROOF_TRANSCRIPT);
    ...
    let mut transcript = Transcript::new(TABAR_TO_AR_PLONK_PROOF_TRANSCRIPT);
...
#[cfg(feature = "parallel")]
pub fn batch_verify_t_abar_to_ar_note<D: Digest<OutputSize = U64> + Default + Sync + Send>(
    params: &VerifierParams,
    notes: &[&AbarToArNote],
