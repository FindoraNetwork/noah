...
impl<G: Group> Default for ACSignature<G> {
  fn default() -> ACSignature<G> {
    ACSignature { sigma1: G::get_identity(),
...
pub fn ac_sign<R: CryptoRng + RngCore, P: Pairing>(prng: &mut R,
                                                   issuer_sk: &ACIssuerSecretKey<P::G1,
                                                                      P::ScalarField>,
                                                   user_pk: &ACUserPublicKey<P::G1>,
                                                   attrs: &[P::ScalarField])
                                                   -> Result<ACSignature<P::G1>, ZeiError> {
  ...
  Ok(ACSignature::<P::G1> { sigma1: issuer_sk.gen1.mul(&u),
...
pub fn ac_commit_with_key<R: CryptoRng + RngCore, P: Pairing>(
  ...
  let sok = prove_pok::<_, P>(&mut transcript,
                              prng,
                              user_sk,
...
pub fn ac_randomize<P: Pairing>(sig: &ACSignature<P::G1>,
                                key: &ACKey<P::ScalarField>)
                                -> ACCommitment<P::G1> {
...
pub fn ac_verify_commitment<P: Pairing>(issuer_pub_key: &ACIssuerPublicKey<P::G1, P::G2>,
...
pub fn ac_reveal<R: CryptoRng + RngCore, P: Pairing>(
  ...
  let attributes = credential.attributes
                             .iter()
...
pub fn ac_verify<P: Pairing>(issuer_pub_key: &ACIssuerPublicKey<P::G1, P::G2>,
...
pub(crate) mod credentials_tests {
  ...
  fn reveal<P: Pairing>(bitmap: &[bool]) {
    ...
    let reveal_sig = super::ac_reveal::<_, P>(&mut prng, &user_sk, &credential, bitmap).unwrap();
  ...
  pub fn to_json_credential_structures<P: Pairing>() {
    ...
    let sig =
      super::ac_sign::<_, P>(&mut prng, &issuer_keys.1, &user_keys.0, attrs.as_slice()).unwrap();
    let credential = Credential { signature: sig,
                                  attributes: attrs,
