...
pub(super) fn cloak_shuffle_gadget<CS: RandomizableConstraintSystem>(
  ...
  cs.specify_randomized_constraints(move |cs| {
      ...
      for (in_var, perm_var) in input.iter().zip(permuted.iter()) {
        ...
        let (single_in, single_pe, _) = cs.multiply(in_var.amount + challenge * in_var.asset_type,
                                                    perm_var.amount
                                                    + challenge * perm_var.asset_type);
...
mod test {
  ...
  fn test_cloak_merge() {
    ...
    let sorted_values =
      vec![CloakValue::new(RistrettoScalar::from_u32(1), RistrettoScalar::from_u32(10)),
    ...
    let mid_values =
      vec![CloakValue::new(RistrettoScalar::from_u32(4), RistrettoScalar::from_u32(10)),
           CloakValue::new(RistrettoScalar::from_u32(2), RistrettoScalar::from_u32(11)),
           CloakValue::new(RistrettoScalar::from_u32(7), RistrettoScalar::from_u32(11))];
    let out_values =
      vec![CloakValue::new(RistrettoScalar::from_u32(0), RistrettoScalar::from_u32(10)),
    ...
    let added = allocate_cloak_vector(&mut prover, Some(&out_values), out_values.len()).unwrap();
    let num_wires =
    ...
    let mut prover_transcript = Transcript::new(b"test");
    let mut prover = Prover::new(&pc_gens, &mut prover_transcript);
    let sorted_coms_vars: Vec<(CloakCommitment, CloakVariable)> =
      sorted_values.iter()
                   .map(|value| {
    ...
    let mid = allocate_cloak_vector(&mut prover, Some(&mid_values), mid_values.len()).unwrap();
    let added = allocate_cloak_vector(&mut prover, Some(&out_values), out_values.len()).unwrap();
    let sorted_vars: Vec<CloakVariable> = sorted_coms_vars.iter().map(|(_, var)| *var).collect();
    let num_wires =
    ...
    let sorted_vars: Vec<CloakVariable> = sorted_coms.iter()
                                                     .map(|com| com.commit_verifier(&mut verifier))
                                                     .collect();
    let mid = allocate_cloak_vector(&mut verifier, None, mid_values.len()).unwrap();
    let added = allocate_cloak_vector(&mut verifier, None, out_values.len()).unwrap();
    let num_wires =
      super::cloak_merge_gadget(&mut verifier, &sorted_vars[..], &mid[..], &added[..]).unwrap();
    let num_wires = num_wires + added.len() + mid.len();
    let bp_gens = BulletproofGens::new(num_wires.next_power_of_two(), 1);
    assert!(verifier.verify(&proof, &pc_gens, &bp_gens).is_ok());
  }
  ...
  fn test_shuffle() {
    let pc_gens = PedersenGens::default();
    let input_values =
      vec![CloakValue::new(RistrettoScalar::from_u32(10), RistrettoScalar::from_u32(10)),
    ...
    let shuffled_values =
      vec![CloakValue::new(RistrettoScalar::from_u32(20), RistrettoScalar::from_u32(20)),
           CloakValue::new(RistrettoScalar::from_u32(40), RistrettoScalar::from_u32(40)),
    ...
    let shuffled =
      allocate_cloak_vector(&mut prover, Some(&shuffled_values), shuffled_values.len()).unwrap();
    let num_wires =
    ...
    let num_wires = super::cloak_shuffle_gadget(&mut prover, input.to_vec(), bad_shuffle).unwrap();
    let num_wires = num_wires + input.len() + shuffled.len();
    let bp_gens = BulletproofGens::new(num_wires.next_power_of_two(), 1);
    let proof = prover.prove(&bp_gens).unwrap();
    ...
    let mut verifier = Verifier::new(&mut verifier_transcript);
    let input = allocate_cloak_vector(&mut verifier, None, input_values.len()).unwrap();
    let bad_shuffle = allocate_cloak_vector(&mut verifier, None, bad_shuffle_values.len()).unwrap();
