    ...
    H: MTHash<S = Sc>,
{
    ...
    let tree = MerkleTree {
        root: create_merkle_node::<Sc, H>(elements, 0),
        size: elements.len(),
    };
    Ok(tree)
}
...
fn create_merkle_node<S: Copy + Debug, H: MTHash<S = S>>(
    elements: &[S],
    level: usize,
) -> MerkleNode<S> {
...
fn prove_node<S: Copy + PartialEq + Eq + Debug>(
    ...
    let (elem, mut v) = prove_node(
        node.right.as_ref().unwrap().as_ref(),
        index - size / 2,
        size / 2,
    );
...
mod test {
    ...
    fn test_mt() {
        ...
        for i in 0..size {
            ...
            let b = mt_verify::<Scalar, MiMCHash>(
                &merkle_root,
                &e.add(&Scalar::from_u32(1)),
                &path[..],
            );
