...
fn trascript_init<G1: Group, G2: Group>(transcript: &mut Transcript,
                                        pc_gens_g1: &PedersenGens<G1>,
                                        pc_gens_g2: &PedersenGens<G2>,
                                        c1: &G1,
                                        c2: &G1,
                                        c3: &G2) {
  transcript.append_message(b"Domain Separation",
                            b"PedersenEq pair to vector in different groups");
  transcript.append_message(b"G1 base",
                            &pc_gens_g1.get_base(0).unwrap().to_compressed_bytes());
  transcript.append_message(b"G1 base blinding",
                            &pc_gens_g1.get_blinding_base().to_compressed_bytes());
  transcript.append_message(b"G2 base 0",
                            &pc_gens_g2.get_base(0).unwrap().to_compressed_bytes());
  transcript.append_message(b"G2 base 1",
                            &pc_gens_g2.get_base(1).unwrap().to_compressed_bytes());
  transcript.append_message(b"G2 base blinding",
                            &pc_gens_g2.get_blinding_base().to_compressed_bytes());
  transcript.append_message(b"Commitment first value in g1", &c1.to_compressed_bytes());
  transcript.append_message(b"Commitment second value in g1", &c2.to_compressed_bytes());
  transcript.append_message(b"Commitment values in g2", &c3.to_compressed_bytes());
}

fn transcript_append_commitments_get_challenge<G1: Group, G2: Group>(transcript: &mut Transcript,
                                                                     com_v1: &G1,
                                                                     com_v2: &G1,
                                                                     com_v1_v2: &G2,
                                                                     n: &BigUint)
...
pub fn prove_pair_to_vector_pc<R: CryptoRng + RngCore, G1: Group, G2: Group>(
  ...
  // input commitments
  let value1_g1 = G1::S::from_le_bytes(values.0).map_err(|_| ZeiError::SerializationError)?;
  let value2_g1 = G1::S::from_le_bytes(values.1).map_err(|_| ZeiError::SerializationError)?;
  let c1 = pc_gens1.commit(&[value1_g1], &blinds_g1.0)?;
  let c2 = pc_gens1.commit(&[value2_g1], &blinds_g1.1)?;

  let value1_g2 = G2::S::from_le_bytes(values.0).map_err(|_| ZeiError::SerializationError)?;
  let value2_g2 = G2::S::from_le_bytes(values.1).map_err(|_| ZeiError::SerializationError)?;
  ...
  let com_v1_v2 = pc_gens2.commit(&[v1_mod_q, v2_mod_q], &b3_scalar)
                          .map_err(|_| ZeiError::ParameterError)?;

  // 3. commit and get challenge
  let challenge =
    transcript_append_commitments_get_challenge(transcript, &com_v1, &com_v2, &com_v1_v2, &n);
  ...
  let responses = [BigNum(value1_buint * &challenge + v1),
...
pub fn verify_pair_to_vector_pc<G1: Group, G2: Group>(transcript: &mut Transcript,
                                                      coms_g1: (&G1, &G1),
                                                      com_g2: &G2,
                                                      pc_gens1: &PedersenGens<G1>,
                                                      pc_gens2: &PedersenGens<G2>,
  ...
  let challenge = transcript_append_commitments_get_challenge(transcript,
                                                              &proof.com_v1,
                                                              &proof.com_v2,
                                                              &proof.com_v1_v2,
                                                              &n);
  ...
  let com_response_value1 = pc_gens1.commit(&[response_value1_mod_p], &response_blind1_mod_p)
  ...
  let com_response_value2 = pc_gens1.commit(&[response_value2_mod_p], &response_blind2_mod_p)
                                    .map_err(|_| ZeiError::ZKProofVerificationError)?;
  ...
  let response_com_v1_v2 = pc_gens2.commit(&[response_value1_mod_q, response_value2_mod_q],
...
fn biguint_mod_scalar<S: Scalar>(bigint: &BigUint) -> S {
  ...
  let bigint_mod_p = bigint.modpow(&BigUint::from(1u64), &scalar_size)
                           .to_bytes_le();
...
mod test {
  ...
  pub fn test_pc_eq() {
    ...
    let proof = super::prove_pair_to_vector_pc(&mut prng,
                                               &mut prover_transcript,
                                               (&value1_g2.to_bytes(), &value2_g2.to_bytes()),
                                               (&blind1, &blind2),
                                               &blind3,
                                               &pc_gens_rist,
                                               &pc_gens_jubjub).unwrap();
