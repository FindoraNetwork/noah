...
pub fn bls_sign<P: Pairing, B: AsRef<[u8]>>(signing_key: &BlsSecretKey<P>,
                                            message: &B)
                                            -> BlsSignature<P> {
...
pub fn bls_verify<P: Pairing, B: AsRef<[u8]>>(ver_key: &BlsPublicKey<P>,
                                              message: &B,
...
pub fn bls_aggregate<P: Pairing>(ver_keys: &[&BlsPublicKey<P>],
...
pub fn bls_verify_aggregated<P: Pairing, B: AsRef<[u8]>>(ver_keys: &[&BlsPublicKey<P>],
                                                         message: &B,
...
pub fn bls_batch_verify<P: Pairing, B: AsRef<[u8]>>(ver_keys: &[BlsPublicKey<P>],
                                                    messages: &[B],
                                                    signatures: &[BlsSignature<P>])
                                                    -> Result<(), ZeiError> {
...
pub fn bls_batch_verify_added_signatures<P: Pairing, B: AsRef<[u8]>>(ver_keys: &[BlsPublicKey<P>],
                                                                     messages: &[B],
                                                                     signature: &BlsSignature<P>)
                                                                     -> Result<(), ZeiError> {
  let a = P::pairing(&P::G1::get_base(), &signature.0);
...
pub fn bls_hash_pubkeys_to_scalars<P: Pairing>(ver_keys: &[&BlsPublicKey<P>])
                                               -> Vec<P::ScalarField> {
...
mod tests {
  ...
  fn bls_aggregated_signatures() {
    ...
    let agg_signature =
      super::bls_aggregate::<Bls12381>(&keys, &[&signature1, &signature2, &signature3]);
