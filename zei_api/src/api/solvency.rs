...
impl SolvencyAudit {
  ...
  pub fn build_prover(&self,
    ...
    for record in asset_oars.iter() {
      if record.blind_asset_record.is_public() {
        prover.public_assets
              .push(SolvencyAudit::get_record_entry_from_oar(&record));
      } else {
        prover.hidden_assets
              .push(SolvencyAudit::get_record_entry_from_oar(&record));
        prover.hidden_assets_blinds
    ...
    for record in liability_oars.iter() {
      if record.blind_asset_record.is_public() {
        prover.public_liabilities
              .push(SolvencyAudit::get_record_entry_from_oar(&record));
      } else {
        prover.hidden_liabilities
              .push(SolvencyAudit::get_record_entry_from_oar(&record));
        prover.hidden_liabilities_blinds
              .push(SolvencyAudit::get_hidden_record_blinds(&record));
      }
    }
  ...
  pub fn build_verifier(&self) -> SolvencyVerifier {
    ...
    for record in self.assets.iter() {
      if record.is_public() {
        verifier.public_assets
                .push(SolvencyAudit::get_record_entry_from_bar(&record));
      } else {
        verifier.hidden_assets_commitments
    ...
    for record in self.liabilities.iter() {
      if record.is_public() {
        verifier.public_liabilities
                .push(SolvencyAudit::get_record_entry_from_bar(&record));
      } else {
        verifier.hidden_liabilities_commitments
                .push(SolvencyAudit::get_hidden_record_commitments(&record));
      }
    }
...
impl SolvencyAudit {
  ...
  fn get_hidden_record_commitments(record: &BlindAssetRecord) -> CloakCommitment {
    ...
    let amount_com = if record.amount.is_confidential() {
      ...
      (amount_com_lo.decompress()
                    .unwrap()
                    .add(&amount_com_hi.decompress()
                                       .unwrap()
                                       .mul(&Scalar::from_u64(1u64 << 32)))).compress()
    } else {
      pc_gens.commit(Scalar::from_u64(record.amount.get_amount().unwrap()),
                     Scalar::from_u32(0))
             .compress()
    };
...
impl SolvencyProver {
  ...
  pub fn prove(&self,
    ...
    let proof = solvency::prove_solvency(&bp_gens,
                                         &pc_gens,
                                         &self.hidden_assets,
...
impl SolvencyVerifier {
  ...
  pub fn verify(&self,
    ...
    solvency::verify_solvency(&bp_gens,
                              &pc_gens,
...
mod test {
  ...
  // helper function
  fn build_bar<R: CryptoRng + RngCore>(pubkey: &XfrPublicKey,
                                       prng: &mut R,
                                       pc_gens: &RistrettoPedersenGens,
                                       amt: u64,
                                       asset_type: AssetType,
  ...
  fn test_solvency_soundness() {
    ...
    let mut audit: SolvencyAudit = Default::default();
    assert!(audit.add_record(SolvencyRecordType::Liability, &bars[1].0)
                 .is_ok()); // 20 * 5 = 100
    assert!(audit.add_record(SolvencyRecordType::Liability, &bars[3].0)
                 .is_ok()); // 40 * 3 = 120
    assert!(audit.add_record(SolvencyRecordType::Asset, &bars[1].0)
                 .is_ok()); // 20 * 5 = 100
    assert!(audit.add_record(SolvencyRecordType::Asset, &bars[4].0)
