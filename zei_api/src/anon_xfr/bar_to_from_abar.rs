...
pub fn gen_bar_to_abar_body<R: CryptoRng + RngCore>(prng: &mut R,
                                                    params: &UserParams,
                                                    record: &OpenAssetRecord,
                                                    abar_keypair: &AXfrKeyPair,
                                                    enc_key: &XPublicKey)
                                                    -> Result<BarToAbarBody, ZeiError> {
  let (open_abar, proof) = bar_to_abar(prng, params, record, abar_keypair, enc_key)?;
  let body = BarToAbarBody { input: record.blind_asset_record.clone(),
...
pub fn gen_bar_to_abar_note<R: CryptoRng + RngCore>(prng: &mut R,
                                                    params: &UserParams,
                                                    record: &OpenAssetRecord,
                                                    bar_keypair: &XfrKeyPair,
                                                    abar_keypair: &AXfrKeyPair,
                                                    enc_key: &XPublicKey)
                                                    -> Result<BarToAbarNote, ZeiError> {
  let body = gen_bar_to_abar_body(prng, params, record, &abar_keypair, enc_key)?;
  let msg = bincode::serialize(&body).map_err(|_| ZeiError::SerializationError)?;
...
pub fn verify_bar_to_abar_body(params: &NodeParams, body: &BarToAbarBody) -> Result<(), ZeiError> {
  verify_bar_to_abar(params, &body.input, &body.output, &body.proof)
}
...
pub fn verify_bar_to_abar_note(params: &NodeParams,
                               note: &BarToAbarNote,
                               bar_pub_key: &XfrPublicKey)
                               -> Result<(), ZeiError> {
  verify_bar_to_abar_body(params, &note.body)?;
  let msg = bincode::serialize(&note.body).map_err(|_| ZeiError::SerializationError)?;
...
pub(crate) fn bar_to_abar<R: CryptoRng + RngCore>(
  ...
  abar_keypair: &AXfrKeyPair,
  enc_key: &XPublicKey)
  -> Result<(OpenAnonBlindAssetRecord, ConvertBarAbarProof), ZeiError> {
  ...
  let commitment_amount_asset_type = pc_gens_jubjub.commit(&[JubjubScalar::from_u64(obar.amount),
                                                             asset_type_scalar],
                                                           &blind)?;

  let ristretto_amount_blind =
    obar.amount_blinds.0.add(&obar.amount_blinds
                                  .1
  ...
  let commitment_eq_proof = prove_pair_to_vector_pc(prng,
                                                    &mut transcript,
                                                    (&obar.amount.to_le_bytes(),
                                                     &asset_type_scalar.to_bytes()),
  ...
  // 3. compute ABAR
  let oabar = OpenAnonBlindAssetRecordBuilder::new().amount(obar.amount)
                                                    .asset_type(obar.asset_type)
                                                    .pub_key(abar_keypair.pub_key())
                                                    .finalize(prng, &enc_key)?
                                                    .build()?;
  // 4. prove abar correctness
  let pc_rescue_commitments_eq_proof =
    prove_eq_committed_vals(prng,
                            params,
  ...
  Ok((oabar,
      ConvertBarAbarProof { commitment_amount_asset_type,
                            commitment_eq_proof,
                            pc_rescue_commitments_eq_proof }))
}

pub(crate) fn verify_bar_to_abar(params: &NodeParams,
                                 bar: &BlindAssetRecord,
                                 abar: &AnonBlindAssetRecord,
                                 proof: &ConvertBarAbarProof)
                                 -> Result<(), ZeiError> {
  let pc_gens_rist = PedersenGens::<RistrettoPoint>::from(bulletproofs::PedersenGens::default());
  let pc_gens_jubjub = PedersenGens::<JubjubPoint>::new(2);
  ...
  let (com_low, com_high) = match bar.amount {
    XfrAmount::Confidential((low, high)) => {
      (low.decompress().ok_or(ZeiError::DecompressElementError)?,
       high.decompress().ok_or(ZeiError::DecompressElementError)?)
    }
    XfrAmount::NonConfidential(amount) => {
      // fake commitment
      let (l, h) = utils::u64_to_u32_pair(amount);
      (pc_gens_rist.commit(&[RistrettoScalar::from_u32(l)], &RistrettoScalar::zero())?,
       pc_gens_rist.commit(&[RistrettoScalar::from_u32(h)], &RistrettoScalar::zero())?)
    }
  };
  ...
  let com_asset_type = match bar.asset_type {
    XfrAssetType::Confidential(a) => a.decompress().ok_or(ZeiError::DecompressElementError)?,
    XfrAssetType::NonConfidential(a) => {
      // fake commitment
      pc_gens_rist.commit(&[a.as_scalar()], &RistrettoScalar::zero())?
    }
  };
  ...
  let mut transcript = Transcript::new(b"Commitment Equality Proof");
  crypto::pc_eq_groups::verify_pair_to_vector_pc(&mut transcript,
  ...
  verify_eq_committed_vals(params,
...
#[cfg(test)]
mod test {
  use crate::anon_xfr::bar_to_from_abar::{gen_bar_to_abar_note, verify_bar_to_abar_note};
  use crate::anon_xfr::keys::AXfrKeyPair;
  ...
  // helper function
  fn build_bar(pubkey: &XfrPublicKey,
               prng: &mut ChaChaRng,
  ...
  fn test_bar_to_abar() {
    ...
    // confidential case
    let (bar_conf, memo) = build_bar(&bar_keypair.pub_key,
                                     &mut prng,
                                     &pc_gens,
                                     10u64,
    ...
    let (oabar_conf, proof_conf) =
      super::bar_to_abar(&mut prng, &params, &obar, &abar_keypair, &enc_key).unwrap();
    let abar_conf = AnonBlindAssetRecord::from_oabar(&oabar_conf);
    // non confidential case
    let (bar_non_conf, memo) =
      build_bar(&bar_keypair.pub_key,
                &mut prng,
                &pc_gens,
                10u64,
                AssetType::from_identical_byte(1u8),
    ...
    // confidential case
    assert!(super::verify_bar_to_abar(&node_params, &bar_conf, &abar_conf, &proof_conf).is_ok());
    // non confidential case
    assert!(super::verify_bar_to_abar(&node_params,
                                      &bar_non_conf,
                                      &abar_non_conf,
                                      &proof_non_conf).is_ok());
  }
  ...
  fn test_bar_to_abar_xfr_note() {
    ...
    let (bar, memo) = build_bar(&bar_keypair.pub_key,
                                &mut prng,
                                &pc_gens,
                                amount,
                                asset_type,
    ...
    let params = UserParams::eq_committed_vals_params();
    let note = gen_bar_to_abar_note(&mut prng,
                                    &params,
                                    &obar,
                                    &bar_keypair,
                                    &abar_keypair,
    ...
    let oabar = OpenAnonBlindAssetRecordBuilder::from_abar(&note.body.output,
    ...
    assert_eq!(abar_keypair.pub_key().randomize(&oabar.key_rand_factor),
