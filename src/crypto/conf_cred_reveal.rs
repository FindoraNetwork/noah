...
pub fn cac_multi_prove<R, S, P>(
    ...
    where R: CryptoRng + Rng, S: Scalar, P: Pairing<S>
{
    Ok(CACProof(agg_pok_attrs_prove::<R,S,P>(
        prng,
        ac_issuer_pub_key,
        recv_enc_pub_keys,
        attrs,
...
/// I verify a CACProof.
pub fn cac_verify<S: Scalar, P: Pairing<S>>(
    ac_issuer_pub_key: &ACIssuerPublicKey<P::G1, P::G2>,
    recv_enc_pub_key: &ElGamalPublicKey<P::G1>,
...
pub fn cac_multi_verify<S: Scalar, P: Pairing<S>>(
    ac_issuer_pub_key: &ACIssuerPublicKey<P::G1, P::G2>,
    recv_enc_pub_keys: &[&ElGamalPublicKey<P::G1>],
    ac_reveal_sigs: &[&ACRevealSig<P::G1, P::G2, S>],
...
pub(crate) fn agg_pok_attrs_prove<R,S,P>(
    prng: &mut R,
    ac_issuer_pub_key: &ACIssuerPublicKey<P::G1, P::G2>,
    recv_enc_pub_keys: &[&ElGamalPublicKey<P::G1>],
    attrs: &[&[S]],
...
) -> Result<(), ZeiError>
{
    ...
    for i in 0..n_attrs{
        ...
        for (pub_key, rand_resp_inst, attr_resp_inst,  ctexts_inst, scalar) in izip!(
               recv_enc_pub_keys.iter(),
               rand_responses.iter(),
               attr_responses.iter(),
               ctexts.iter(),
               lc_scalars.iter()) {
        ...
        agg_ctexts.push(ElGamalCiphertext{
            e1: sum_e1,
            e2: sum_e2,
        })
    }
    ...
    for (ctext, attr_com, rand_coms_g, rand_coms_pk, g_x_attr_resp, g_x_rand_resp, pk_x_rand_resp)
        in izip!(agg_ctexts, attr_commitments, rand_commitments_g, rand_commitments_pk, agg_g_x_attr_resp, agg_g_x_rand_resp, agg_pk_x_rand_resp){
        let e1 = &ctext.e1;
        let e2 = &ctext.e2;
...
) -> Result<(), ZeiError>
{
    ...
    let mut ck_vec = vec![];
    for (lc_scalar_k, reveal_sig_k, attr_sum_com_k, attr_resp_k)
        in izip!(lc_scalars, reveal_sigs, attr_sum_com_yy2, attr_resps) {

        let c_k = ac_challenge::<S, P>(issuer_pub_key, &reveal_sig_k.sig, &reveal_sig_k.pok.commitment)?;

        let hidden_k = ac_vrfy_hidden_terms_addition::<S,P>(
            &c_k,
            reveal_sig_k,
...
mod test{
    ...
    fn gen_ac_reveal_sig<S: Scalar, P: Pairing<S>>(
        ...
        reveal_bitmap: &[bool])
        -> (Vec<ElGamalCiphertext<P::G1>>, Vec<S>, Vec<S>, ACRevealSig<P::G1, P::G2, S>, ElGamalPublicKey<P::G1>)
    {
    ...
    pub(super) fn confidential_reveal_agg<S: Scalar, P: Pairing<S>>(reveal_bitmap: &[bool]){
        ...
        let mut cac_proof = cac_multi_prove::<_, S, P>(
            &mut prng,
            &ac_issuer_pub_key,
    ...
    pub(super) fn confidential_ac_reveal<S: Scalar, P: Pairing<S>>(reveal_bitmap: &[bool]){
        ...
        let mut cac_proof = cac_prove::<_, S, P>(
            &mut prng,
            ac_issuer_pub_key,
            &recv_enc_pub_key,
...
mod test_serialization{
    ...
    use crate::algebra::groups::{Group, Scalar};
    use super::{CACProof, AggPoKAttrs};
    ...
    fn to_json<G1: Group<S>, G2: Group<S>, S: Scalar>(){
        ...
        let cac_proof = CACProof(AggPoKAttrs{ //2 instances 3 attributes
            ...
            rands_resps: vec![
                vec![S::from_u32(60), S::from_u32(40), S::from_u32(20)],
                vec![S::from_u32(70), S::from_u32(50), S::from_u32(30)],
            ],
        });
    ...
    fn to_msg_pack<G1: Group<S>, G2: Group<S>, S: Scalar>(){
        ...
        //keys serialization
        let mut vec = vec![];
        cac_proof.serialize(&mut rmp_serde::Serializer::new(&mut vec)).unwrap();
