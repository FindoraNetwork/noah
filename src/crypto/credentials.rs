...
pub fn ac_sign<R: CryptoRng + Rng, S:Scalar, P: Pairing<S>>(
    prng: &mut R,
...
pub(crate) fn ac_challenge<S: Scalar, P: Pairing<S>>(
    issuer_pub_key: &ACIssuerPublicKey<P::G1, P::G2>,
    sig: &ACSignature<P::G1>,
    commitment: &P::G2) -> Result<S, ZeiError>
{
...
pub fn ac_verify<S: Scalar, P: Pairing<S>>(
    issuer_pub_key: &ACIssuerPublicKey<P::G1, P::G2>,
    revealed_attrs: &[S],
    bitmap: &[bool],
...
{
    let challenge = ac_challenge::<S,P>(issuer_pub_key, &reveal_sig.sig, &reveal_sig.pok.commitment)?;
    // hidden = X_2*c - proof_commitment + &G2 * r_t + Z2 * r_sk + \sum r_attr_i * Y2_i;
    let hidden = ac_vrfy_hidden_terms_addition::<S,P>(&challenge, &reveal_sig, issuer_pub_key, bitmap)?;
    // revealed = c * \sum attr_j * Y2_j;
    let revealed = ac_vrfy_revealed_terms_addition::<S,P>(&challenge, revealed_attrs, issuer_pub_key, bitmap)?;
    ...
    let rhs = P::pairing(
        &reveal_sig.sig.sigma2.mul(&challenge),
        &issuer_pub_key.gen2
    );
...
pub(crate) fn ac_vrfy_hidden_terms_addition<S: Scalar, P: Pairing<S>>(
    ...
    reveal_sig: &ACRevealSig<P::G1, P::G2, S>,
    issuer_pub_key: &ACIssuerPublicKey<P::G1, P::G2>,
    bitmap: &[bool]) -> Result<P::G2, ZeiError>
{
    //compute X_2 * challenge - commitment + G2 * &response_t + PK.Z2 * response_sk +
    // sum PK.Y2_i * response_attr_i
    let mut q = issuer_pub_key.xx2.mul(&challenge).sub(&reveal_sig.pok.commitment); //X_2*challenge - proof.commitment
...
fn ac_vrfy_revealed_terms_addition<S: Scalar, P:Pairing<S>>(
    challenge: &S,
    revealed_attrs: &[S],
    issuer_pub_key: &ACIssuerPublicKey<P::G1, P::G2>,
    bitmap: &[bool]) -> Result<P::G2, ZeiError>
...
pub(crate) mod credentials_tests {
    ...
    fn reveal<S: Scalar, P: Pairing<S>>(bitmap: &[bool]){
        ...
        let issuer_keypair =
            super::ac_keygen_issuer::<_,S, P>(&mut prng, n);
        let issuer_pk = &issuer_keypair.0;
        let issuer_sk = &issuer_keypair.1;
        let (user_pk, user_sk) =
            super::ac_keygen_user::<_, S, P>(&mut prng, issuer_pk);
        ...
        let sig =
            super::ac_sign::<_, S,P>(&mut prng, &issuer_sk, &user_pk, attrs.as_slice());

        let reveal_sig = super::ac_reveal::<_, S,P>(
            &mut prng,
            &user_sk,
            issuer_pk,
            &sig,
    ...
    pub fn to_json_credential_structures<S:Scalar, P: Pairing<S>>(){
        ...
        let issuer_pub_key_de: ACIssuerPublicKey<P::G1, P::G2> = serde_json::from_str(&json_str).unwrap();
        ...
        let credential =
            super::ac_sign::<_, S,P>(&mut prng, &issuer_keys.1, &user_keys.0, &attrs);
        let reveal_sig = super::ac_reveal::<_, S,P>(
            &mut prng,
            &user_keys.1,
            &issuer_keys.0,
    ...
    pub fn to_msg_pack_credential_structures<S: Scalar, P: Pairing<S>>(){
        ...
        let issuer_pub_key_de: ACIssuerPublicKey<P::G1, P::G2> = Deserialize::deserialize(&mut de).unwrap();
        assert_eq!(issuer_keys.0, issuer_pub_key_de);

        let mut vec = vec![];
        issuer_keys.1.serialize(&mut rmp_serde::Serializer::new(&mut vec)).unwrap();
        let mut de = Deserializer::new(&vec[..]);
        let issuer_priv_key_de: ACIssuerSecretKey<P::G1, S> = Deserialize::deserialize(&mut de).unwrap();
        ...
        let mut vec = vec![];
        reveal_sig.serialize(&mut rmp_serde::Serializer::new(&mut vec)).unwrap();
        let mut de = Deserializer::new(&vec[..]);
