use bulletproofs_yoloproof::r1cs::{ConstraintSystem, RandomizableConstraintSystem,
                                   RandomizedConstraintSystem, Variable, R1CSError, LinearCombination};
...
) -> Result<usize, R1CSError>
{
    ...
    cs.specify_randomized_constraints(move |cs| {
        ...
        for (in_var, perm_var) in input_pairs.iter().zip(permuted_pairs.iter()){
            ...
            let (single_in, single_pe, _) = cs.multiply(in_var.0 + challenge * in_var.1, perm_var.0 + challenge * perm_var.1);
...
#[cfg(test)]
mod test{
    use bulletproofs_yoloproof::r1cs::{Prover, Verifier, Variable};
    ...
    fn test_list_mix(){
        ...
        let out_values = [
            (Scalar::from(0u8), Scalar::from(10u8)),
            (Scalar::from(4u8), Scalar::from(10u8)),
        ...
        let sorted_coms_vars: Vec<((CompressedRistretto, CompressedRistretto),(Variable, Variable))>
            = sorted_values.iter().map(|(amount, asset_type)| {
            let (sorted_a_com, sorted_a_var) = prover.commit(*amount, Scalar::from(1u8));
            let (sorted_t_com, sorted_t_var) = prover.commit(*asset_type, Scalar::from(2u8));
        ...
        let sorted_coms: Vec<(CompressedRistretto, CompressedRistretto)> =
            sorted_coms_vars.iter().map(|((a_c, t_c), _)| (*a_c, *t_c)).collect();
        let sorted_vars: Vec<(Variable, Variable)> = sorted_coms.iter().map(|(a_c, t_c)| {
            (verifier.commit(*a_c), verifier.commit(*t_c))
        }).collect();
        let mid = allocate_vector(&mut verifier, &mid_values.to_vec());
        let added = allocate_vector(&mut verifier, &out_values.to_vec());
