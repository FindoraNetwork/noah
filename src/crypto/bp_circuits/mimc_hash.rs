...
pub(crate) fn mimc_func<CS: ConstraintSystem>(cs: &mut CS, x: LinearCombination, c: Scalar) -> Result<(Variable, usize), R1CSError>
{
    let x_plus_c = x + c;
...
fn feistel_round<CS: ConstraintSystem>(cs: &mut CS, x: LinearCombination, y: LinearCombination, c: Scalar) -> Result<(LinearCombination, LinearCombination, usize), R1CSError>
{
    let new_y = x.clone();
...
pub(crate) fn feistel_network<CS: ConstraintSystem>(cs: &mut CS, x: LinearCombination, y: LinearCombination, c: &[Scalar]) -> Result<(LinearCombination, LinearCombination, usize), R1CSError>
{
...
pub(crate) fn mimc_hash<CS: ConstraintSystem>(cs: &mut CS, values: &[LinearCombination], level: usize) -> Result<(LinearCombination, usize), R1CSError>{
...
pub fn hash_proof<CS: ConstraintSystem>(cs: &mut CS, x: Variable, y: Variable, out: Variable) -> Result<usize, R1CSError>{
...
mod test{
    ...
    fn test_mimc_fn() {
        ...
        let (ver_out, num_left_wires) = super::mimc_func(&mut verifier, ver_x.into(), scalar_c).unwrap();
    ...
    fn test_feistel_network() {
        ...
        let (outx,outy, num_left_wires) = super::feistel_network(&mut prover, x.into(), y.into(), &scalar_c).unwrap();
        ...
        let (ver_out_x,ver_out_y, num_left_wires) = super::feistel_network(&mut verifier, ver_x.into(), ver_y.into(), &scalar_c).unwrap();
