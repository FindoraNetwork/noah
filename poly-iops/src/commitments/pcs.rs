...
pub trait PolyComScheme {
  ...
  fn batch_prove_eval(&self,
    ...
    for ((z_i_bar, opening_i), value) in z_i_bar_vec.iter()
                                                    .zip(openings.iter())
    ...
    let (g_value, g_proof) = self.prove_eval(transcript, &g_opening, &rho, max_degree)
                                 .map_err(|_| ())?;
    if !g_value.is_zero() {
      Err(())
    } else {
      Ok((eval_values,
          BatchProofEval { commitment: C_q,
                           eval_proof: g_proof }))
    }
  }
  ...
  fn init_pcs_batch_eval_transcript(transcript: &mut Transcript,
                                    max_degree: usize,
                                    points: &[Self::Field],
                                    params: Option<BatchEvalParams<Self::Commitment,
                                                           Self::Field>>) {
  ...
  fn transcript_append_params(transcript: &mut Transcript,
                              max_degree: usize,
                              points: &[Self::Field],
...
mod test {
  ...
  fn test_pcs_eval() {
    ...
    {
      let mut transcript = Transcript::new(b"TestPCS");
      assert!(pcs.verify_eval(&mut transcript, &com, degree, &point, &eval, &proof)
                 .is_ok());
    }
  }
  ...
  fn test_pcs_batch_eval() {
    ...
    let (evals, proof) = {
      let mut transcript = Transcript::new(b"TestPCS");
      let params = BatchEvalParams { commitments: &[&com1, &com2, &com3],
                                     evals: &[] };
      pcs.batch_prove_eval(&mut transcript,
                           &[&open1, &open2, &open3],
                           &points,
                           degree,
                           Some(params))
    ...
    assert_eq!(evals,
               vec![pcs.eval_opening(&open1, &points[0]),
                    ...
                    pcs.eval_opening(&open3, &points[2])]);
    {
      ...
      let params = BatchEvalParams { commitments: &[&com1, &com2, &com3],
                                     evals: &[] };
      assert!(pcs.batch_verify_eval(&mut transcript,
                                    &[&com1, &com2, &com3],
                                    degree,
                                    &points,
                                    &evals,
  ...
  fn test_pcs_batch_eval_simulate_plonk() {
    ...
    assert_eq!(evals,
               vec![pcs.eval_opening(&openf1, &points[0]),
                    ...
                    pcs.eval_opening(&openSigma, &points[7])]);
    {
      let mut transcript = Transcript::new(b"TestPCS");
      assert!(pcs.batch_verify_eval(&mut transcript,
                                    &[&comf1, &comf2, &comf3, &comperm1, &comperm2, &comQ,
                                      &comL, &comSigma],
                                    degree, //Q.degree()
                                    &points,
                                    &evals,
