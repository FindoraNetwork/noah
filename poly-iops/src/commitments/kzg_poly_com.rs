...
pub const BLS_12_381_MODULUS: &str =
...
impl<'a, G> ToBytes for KZGCommitment<G> where G: Group
{
...
impl HomomorphicPolyComElem for KZGCommitment<BLSG1> {
  ...
  fn get_base() -> Self {
    KZGCommitment { value: BLSG1::get_base() }
  }

  fn get_identity() -> Self {
    KZGCommitment { value: BLSG1::get_identity() }
  }

  fn op(&self, other: &Self) -> Self {
    KZGCommitment { value: self.value.add(&other.value) }
  }
  ...
  fn exp(&self, exp: &BLSScalar) -> Self {
    KZGCommitment { value: self.value.mul(exp) }
  }
  ...
  fn inv(&self) -> Self {
    ...
    KZGCommitment { value: self.value.mul(&minus_one_scalar) }
  }
}
...
impl<P: Pairing> KZGCommitmentScheme<P> {
  ...
  pub fn new<R: CryptoRng + RngCore>(max_degree: usize, prng: &mut R) -> KZGCommitmentScheme<P> {
    ...
    KZGCommitmentScheme { public_parameter_group_1,
...
impl<'b> PolyComScheme for KZGCommitmentSchemeBLS {
  ...
  fn commit(&self,
    ...
    let pub_param_group_1_as_ref: Vec<&BLSG1> =
      self.public_parameter_group_1[0..pol_degree + 1].iter()
                                                      .collect();
    let commitment_value = BLSG1::vartime_multi_exp(&coefs_poly_bls_scalar_ref[..],
                                                    &pub_param_group_1_as_ref[..]);

    Ok((KZGCommitment { value: commitment_value }, polynomial))
  }
  ...
  fn eval_opening(&self, opening: &FpPolynomial<Self::Field>, point: &Self::Field) -> Self::Field {
    opening.eval(point)
  }
  ...
  fn verify_eval(&self,
    ...
    let right_pairing_eval =
      algebra::bls12_381::Bls12381::pairing(&proof.0, &x_minus_point_group_element_group_2);
...
mod tests_kzg_impl {
  ...
  fn _check_public_parameters_generation<P: Pairing>() {
    ...
    for i in 0..param_size - 1 {
      ...
      assert_eq!(elem_next_group_1_target_recomputed,
    ...
    assert_eq!(elem_next_group_2_target_recomputed,
  ...
  pub fn test_homomorphic_poly_com_elem() {
    ...
    assert_eq!(commitment_poly1_mult_5.value,
  ...
  fn test_eval() {
    ...
    let res = pcs.prove_eval(&mut not_needed_transcript,
                             &opening,
                             &point,
                             wrong_max_degree);
    assert!(res.is_err());

    let (value, proof) = pcs.prove_eval(&mut not_needed_transcript, &opening, &point, max_degree)
                            .unwrap();
    assert_eq!(value, seven);

    let res = pcs.verify_eval(&mut not_needed_transcript,
                              &commitment_value,
                              degree,
                              &point,
                              &value,
    ...
    let wrong_value_verif = pcs.verify_eval(&mut not_needed_transcript,
                                            &commitment_value,
                                            degree,
                                            &point,
                                            &wrong_value_verif,
