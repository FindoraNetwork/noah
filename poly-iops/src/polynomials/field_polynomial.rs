...
impl<F: Scalar> FpPolynomial<F> {
  ...
  /// use poly_iops::polynomials::field_polynomial::FpPolynomial;
  /// use algebra::bls12_381::BLSScalar;
  /// use algebra::groups::{Zero, One, ScalarArithmetic};
  /// let zero = BLSScalar::zero();
  /// let one = BLSScalar::one();
  /// let two = one.add(&one);
  ...
  pub fn lagrange_ith_base(points: &[F], i: usize) -> Option<Self> {
    ...
    let points_skip_i: Vec<&F> = points.iter()
                                       .enumerate()
                                       .filter(|(j, _)| *j != i)
                                       .map(|(_, v)| v)
                                       .collect();
...
fn recursive_fft<F: Scalar>(coefs: &[&F], root: &F) -> Vec<F> {
  ...
  let even: Vec<&F> = coefs.iter()
                           .enumerate()
                           .filter(|(i, _)| i % 2 == 0)
                           .map(|(_, c)| *c)
                           .collect();
  let odd: Vec<&F> = coefs.iter()
                          .enumerate()
                          .filter(|(i, _)| i % 2 == 1)
                          .map(|(_, c)| *c)
                          .collect();
...
pub fn primitive_nth_root_of_unity<F: Scalar>(num_points: usize) -> Option<F> {
  let q_minus_one = BigUint::from_bytes_le(F::get_field_size_lsf_bytes().as_slice()).sub(1u64);
...
pub fn recursive_ifft<F: Scalar>(values: &[&F], root: &F) -> Vec<F> {
  ...
  recursive_fft(values, &root_inv).into_iter()
                                  .map(|x| {
                                    let mut a = n_inv;
                                    a.mul_assign(&x);
                                    a
