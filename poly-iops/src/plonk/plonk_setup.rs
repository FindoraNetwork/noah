...
pub trait ConstraintSystem {
  ...
  /// constraint equation.
  fn eval_selector_multipliers(&self,
                               wire_vals: &[&Self::Field])
...
impl<F: Scalar> PlonkConstraintSystem<F> {
  pub fn new(num_vars: usize) -> PlonkConstraintSystem<F> {
    PlonkConstraintSystem { selectors: vec![vec![], vec![], vec![], vec![], vec![]], // q_L, q_R, q_M, q_O, q_C
                            wiring: [vec![], vec![], vec![]],
                            num_vars,
                            size: 0,
  ...
  pub fn insert_add_gate(&mut self,
                         left_var_index: usize,
                         right_var_index: usize,
                         out_var_index: usize) {
    self.insert_add_gate_with_inputs_multiplier(left_var_index,
                                                right_var_index,
                                                out_var_index,
                                                F::one(),
                                                F::one())
  }
  pub fn insert_add_gate_with_inputs_multiplier(&mut self,
                                                left_var_index: usize,
                                                right_var_index: usize,
                                                out_var_index: usize,
                                                left_var_multiplier: F,
                                                right_var_multiplier: F) {
    assert!(left_var_index < self.num_vars,
            "Variable index out of Constraint System bound");
    assert!(right_var_index < self.num_vars,
            "Variable index out of Constraint System bound");
    assert!(out_var_index < self.num_vars,
            "Variable index out of Constraint System bound");
  ...
  pub fn insert_mul_gate(&mut self,
                         left_var_index: usize,
                         right_var_index: usize,
                         out_var_index: usize) {
    self.insert_mul_gate_with_input_multiplier(left_var_index,
                                               right_var_index,
                                               out_var_index,
                                               F::one())
  }

  pub fn insert_mul_gate_with_input_multiplier(&mut self,
                                               left_var_index: usize,
                                               right_var_index: usize,
                                               out_var_index: usize,
  ...
  pub fn verify_witness(&self, witness: &[F], online_vars: &[F]) -> Result<(), ()> {
    ...
    for cs_index in 0..self.size() {
      ...
      let add = left_wire_value.mul(&self.selectors[0][cs_index])
                               .add(&right_wire_value.mul(&self.selectors[1][cs_index]));
      let mul = right_wire_value.mul(left_wire_value)
...
pub fn choose_ks<R: CryptoRng + RngCore, F: Scalar>(prng: &mut R,
...
pub type ProverParams<PCS> = PlonkProverParams<<PCS as PolyComScheme>::Opening,
...
pub fn preprocess_prover<PCS: PolyComScheme, CS: ConstraintSystem<Field = PCS::Field>>(
  ...
  let root_m = primitive_nth_root_of_unity::<PCS::Field>(m).ok_or(PlonkError::GroupNotFound(m))?;
  ...
  let Z_H_inv_coset_evals = Z_H.coset_fft_with_unity_root(&root_m, m, &k[1])
                               .into_iter()
                               .map(|x| x.inv().unwrap())
                               .collect();
  ...
  let verifier_params = PlonkVerifierParams { selectors: verifier_selectors,
                                              extended_permutations: verifier_extended_perms,
                                              k,
                                              root,
                                              cs_size: n,
                                              public_vars_constraint_indices:
                                                cs.public_vars_constraint_indices().to_vec(),
                                              lagrange_constants };

  Ok(PlonkProverParams { selectors: prover_selectors,
...
mod test {
  use crate::ioputils::u8_lsf_slice_to_u64_lsf_le_vec;
  use crate::plonk::plonk_setup::{choose_ks, ConstraintSystem, PlonkConstraintSystem};
  ...
  #[test]
  fn test_permutation() {
    let cs = PlonkConstraintSystem::<F> { selectors: vec![vec![], vec![], vec![], vec![]],
