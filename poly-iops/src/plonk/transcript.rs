...
pub(crate) fn transcript_init_plonk<C: ToBytes, F: Scalar>(transcript: &mut Transcript,
...
pub(crate) fn transcript_get_challenge_field_elem<F: Scalar>(transcript: &mut Transcript,
                                                             group_order: usize,
                                                             label: &'static [u8])
                                                             -> F {
  let mut buff = [0u8; 32];
...
pub(crate) fn transcript_get_plonk_challenge_alpha<F: Scalar>(transcript: &mut Transcript,
                                                              group_order: usize)
                                                              -> F {
  transcript_get_challenge_field_elem(transcript, group_order, b"alpha")
}

pub(crate) fn transcript_get_plonk_challenge_beta<F: Scalar>(transcript: &mut Transcript,
                                                             group_order: usize)
                                                             -> F {
  transcript_get_challenge_field_elem(transcript, group_order, b"beta")
}

pub(crate) fn transcript_get_plonk_challenge_gamma<F: Scalar>(transcript: &mut Transcript,
                                                              group_order: usize)
                                                              -> F {
  transcript_get_challenge_field_elem(transcript, group_order, b"gamma")
}

pub(crate) fn transcript_get_plonk_challenge_delta<F: Scalar>(transcript: &mut Transcript,
                                                              group_order: usize)
                                                              -> F {
