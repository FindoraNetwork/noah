...
pub struct ExtendedPointVar(PointVar, ExtendedPoint);
...
impl From<&ExtendedPoint> for Point {
  fn from(point: &ExtendedPoint) -> Point {
    let affine_point = AffinePoint::from(point);
    Point(BLSScalar::new(affine_point.get_u()),
...
const EDWARDS_D: [u8; 32] = [0xb1, 0x3e, 0x34, 0xd6, 0xd6, 0x5f, 0x06, 0x01, 0x26, 0x9d, 0x57,
                             0x37, 0x6d, 0x7f, 0x2d, 0x29, 0xd4, 0x7f, 0xbd, 0xe6, 0x07, 0x92,
                             0xfd, 0xf5, 0x48, 0x2b, 0xfa, 0x4b, 0xe7, 0x18, 0x93, 0x2a];
...
fn compute_base_multiples(base: ExtendedPoint, n: usize) -> Vec<Vec<ExtendedPoint>> {
...
impl TurboPlonkConstraintSystem<BLSScalar> {
  ...
  fn insert_ecc_add_gate(&mut self, p1_var: &PointVar, p2_var: &PointVar, p_out_var: &PointVar) {
    ...
    assert!(p_out_var.0 < self.num_vars,
            "p_out.x variable index out of bound");
    assert!(p_out_var.1 < self.num_vars,
  ...
  fn ecc_add(&mut self,
             p1_var: &PointVar,
             p2_var: &PointVar,
             p1_ext: &ExtendedPoint,
             p2_ext: &ExtendedPoint)
             -> ExtendedPointVar {
    assert!(p1_var.0 < self.num_vars, "p1.x variable index out of bound");
    assert!(p1_var.1 < self.num_vars, "p1.y variable index out of bound");
    assert!(p2_var.0 < self.num_vars, "p2.x variable index out of bound");
  ...
  fn ecc_select(&mut self,
    ...
    let one = BLSScalar::one();
    let zero = BLSScalar::zero();
    let p_out_ext: ExtendedPoint = match (self.witness[b0_var] == one, self.witness[b1_var] == one)
    {
      (false, false) => ExtendedPoint::identity(),
      (true, false) => *G1,
      (false, true) => *G2,
      (true, true) => *G3,
    };
  ...
  pub fn scalar_mul_with_bases(&mut self,
                               bases1: &[ExtendedPoint],
                               bases2: &[ExtendedPoint],
                               bases3: &[ExtendedPoint],
    ...
    let mut p_var_ext = self.ecc_select(&bases1[0],
                                        &bases2[0],
                                        &bases3[0],
                                        b_scalar_var[0],
                                        b_scalar_var[1]);
    for i in 1..n_bits_half {
      let tmp_var_ext = self.ecc_select(&bases1[i],
                                        &bases2[i],
                                        &bases3[i],
                                        b_scalar_var[2 * i],
                                        b_scalar_var[2 * i + 1]);
...
mod test {
  ...
  use jubjub::{AffinePoint, ExtendedPoint, Fq as JubjubField, Fr as JubjubScalar};
  ...
  fn test_ecc_add() {
    ...
    let p1_ext = ExtendedPoint::from(GENERATOR);
    ...
    assert!(cs.verify_witness(&[p1_point.0,
                                p1_point.1,
                                p2_point.0,
                                p2_point.1,
                                p3_double_point.0,
  ...
  fn test_scalar_mul() {
    ...
    let scalar_bytes = [17, 144, 47, 113, 34, 14, 11, 207, 13, 116, 200, 201, 17, 33, 101, 116, 0,
    ...
    let p_out_ext = base_ext * jubjub_scalar;
