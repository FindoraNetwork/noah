...
impl<F: Scalar> ConstraintSystem for TurboPlonkConstraintSystem<F> {
  ...
  fn eval_gate_func(&self,
    ...
    let ecc = sel_vals[7].mul(&wire_vals[0])
                         .mul(&wire_vals[1])
                         .mul(&wire_vals[2])
                         .mul(&wire_vals[3])
                         .mul(&wire_vals[4]);
...
impl<F: Scalar> TurboPlonkConstraintSystem<F> {
  ...
  pub fn new() -> TurboPlonkConstraintSystem<F> {
    ...
    TurboPlonkConstraintSystem { selectors,
  ...
  pub fn insert_lc_gate(&mut self,
                        wires_in: &[VarIndex; 4],
                        wire_out: VarIndex,
                        q1: F,
                        q2: F,
                        q3: F,
                        q4: F) {
    assert!(wires_in.iter().all(|&x| x < self.num_vars),
  ...
  pub fn linear_combine(&mut self,
                        wires_in: &[VarIndex; 4],
                        q1: F,
                        q2: F,
                        q3: F,
  ...
  pub fn range_check(&mut self, var: VarIndex, n_bits: usize) -> Vec<VarIndex> {
    ...
    let b: Vec<VarIndex> = binary_repr.into_iter()
                                      .take(n_bits)
    ...
    match (n_bits - 1) - 3 * m {
      ...
      _ => self.insert_lc_gate(&[acc, b[2], b[1], b[0]],
                               var,
                               bin[3],
                               bin[2],
                               bin[1],
  ...
  pub fn is_equal(&mut self, left_var: VarIndex, right_var: VarIndex) -> VarIndex {
    ...
    let zero = F::zero();
    let inv_diff_scalar = self.witness[diff].inv().unwrap_or(zero);
  ...
  fn push_rescue_selectors(&mut self, q_hash_1: F, q_hash_2: F, q_hash_3: F, q_hash_4: F) {
  ...
  pub fn verify_witness(&self, witness: &[F], online_vars: &[F]) -> Result<(), String> {
    if witness.len() != self.num_vars {
      return Err(format!("witness len = {}, num_vars = {}",
                         witness.len(),
                         self.num_vars));
    }
    ...
    for cs_index in 0..self.size() {
      ...
      for ((c_i, w_i), online_var) in self.public_vars_constraint_indices
                                          .iter()
                                          ...
                                          .zip(online_vars.iter())
      {
        if *c_i == cs_index {
          ...
          if witness[*w_i] != *online_var {
            return Err(format!("cs index {}: online var {:?} does not match witness {:?}",
      ...
      let sel_vals: Vec<&F> = (0..self.num_selectors()).map(|i| &self.selectors[i][cs_index])
                                                       .collect();
      ...
      if eval_gate != F::zero() {
        return Err(format!("cs index {}: wire_vals = ({:?}), sel_vals = ({:?})",
...
mod test {
  ...
  fn test_select() {
    ...
    assert!(cs.verify_witness(&[num[0], num[1], num[2], num[3], num[2], num[3]], &[])
              .is_ok());

    // Set bit0 = 1 and bit1 = 0
    assert!(cs.verify_witness(&[num[1], num[0], num[2], num[3], num[3], num[2]], &[])
              .is_ok());
  ...
  fn test_turbo_plonk_circuit_1() {
    ...
    // set e_binary = [1, 1, 1]
    assert!(cs.verify_witness(&[num[1], num[2], num[3], num[2], eight, num[1], num[1], num[1]],
                              &[])
              .is_err());
  ...
  fn test_turbo_plonk_circuit_2() {
    ...
    let verify = cs.verify_witness(&[num[0].clone(),
                                     num[2].clone(),
                                     num[2].clone(),
                                     num[1].clone(),
                                     num[5].clone(),
    ...
    assert!(cs.verify_witness(&[num[2].clone(),
                                num[0].clone(),
                                num[2].clone(),
                                num[1].clone(),
                                num[5].clone(),
    ...
    assert!(cs.verify_witness(&[num[0].clone(),
                                num[3].clone(),
                                num[4].clone(),
                                num[0].clone(),
                                num[7].clone(),
...
mod turbo_plonk_proofs_test {
  ...
  fn test_turbo_plonk_with_constant_and_online_values<PCS: PolyComScheme,
                                                        R: CryptoRng + RngCore>(
    pcs: &PCS,
    prng: &mut R) {
  ...
  fn test_turbo_plonk_arithmetic_gates<PCS: PolyComScheme, R: CryptoRng + RngCore>(pcs: &PCS,
                                                                                   prng: &mut R) {
    let mut cs = TurboPlonkConstraintSystem::new();
    ...
    let c_idx = cs.add(0, 1);
    let d_idx = cs.mul(0, 1);
    let e_idx = cs.linear_combine(&[0, 1, c_idx, d_idx],
                                  num[2].clone(),
                                  num[3].clone(),
                                  num[1].clone(),
                                  num[1].clone());
  ...
  fn test_turbo_plonk_ecc_gates<PCS: PolyComScheme<Field = BLSScalar>, R: CryptoRng + RngCore>(pcs: &PCS,
                                                                                               prng: &mut R)
  {
    let mut cs = TurboPlonkConstraintSystem::new();
  ...
  fn test_turbo_plonk_rescue_gates<PCS: PolyComScheme<Field = BLSScalar>,
                                     R: CryptoRng + RngCore>(
    pcs: &PCS,
    prng: &mut R) {
    ...
    let online_vars = [PCS::Field::from_str("52184923318241479436224725218017640784400243367974222506608059144773855444730").unwrap()];
    let witness = cs.get_and_clear_witness();
    assert!(cs.verify_witness(&witness, &online_vars).is_ok());
    check_turbo_plonk_proof(pcs, prng, &cs, &witness[..], &online_vars[..]);
  }

  fn check_turbo_plonk_proof<PCS: PolyComScheme, R: CryptoRng + RngCore>(pcs: &PCS,
                                                                         prng: &mut R,
