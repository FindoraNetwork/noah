...
pub fn build_group<F: Scalar>(generator: &F, max_elems: usize) -> Result<Vec<F>, PlonkError> {
  let mut elems = vec![];
  elems.push(F::one());
...
impl<F: Scalar> PlonkChallenges<F> {
  pub(super) fn new() -> PlonkChallenges<F> {
    PlonkChallenges { challenges: Vec::with_capacity(4) }
  }
...
pub(super) fn PublicVars_polynomial<PCS: PolyComScheme>(params: &ProverParams<PCS>,
  ...
  for (i, _) in params.group.iter().enumerate() {
    if let Some((pos, _)) = params.verifier_params
...
pub(super) fn hide_polynomial<R: CryptoRng + RngCore, F: Scalar>(prng: &mut R,
...
pub(super) fn Sigma_polynomial<PCS: PolyComScheme, CS: ConstraintSystem<Field = PCS::Field>>(
  ...
  fn p_of_x<Field: Scalar>(perm_value: usize, n: usize, group: &[Field], k: &[Field]) -> Field {
  ...
  for i in 0..n_constraints - 1 {
    ...
    for j in 0..n_wires_per_gate {
      ...
      let p_x = p_of_x::<PCS::Field>(perm[j * n_constraints + i], n_constraints, group, k);
...
pub(super) fn Quotient_polynomial<PCS: PolyComScheme, CS: ConstraintSystem<Field = PCS::Field>>(
  ...
  let witness_polys_coset_evals: Vec<Vec<PCS::Field>> =
    witness_polys.iter()
  ...
  for point in 0..m {
    let wire_vals: Vec<&PCS::Field> =
      witness_polys_coset_evals.iter()
                               .map(|poly_coset_evals| &poly_coset_evals[point])
                               .collect();
    let sel_vals: Vec<&PCS::Field> = params.selectors_coset_evals
                                           .iter()
                                           .map(|poly_coset_evals| &poly_coset_evals[point])
                                           .collect();
    ...
    for j in 0..cs.n_wires_per_gate() {
      let tmp =
        witness_polys_coset_evals[j][point].add(&delta)
    ...
    for (w_poly_coset_evals, perm_coset_evals) in
      witness_polys_coset_evals.iter()
                               .zip(params.perms_coset_evals.iter())
    {
      let tmp = &w_poly_coset_evals[point].add(&delta)
                                          .add(&gamma.mul(&perm_coset_evals[point]));
      term3.mul_assign(&tmp);
    }

    // alpha^2 * (Sigma(X) - 1) * L_1(X)
    let term4 = alpha_sq.mul(&params.L1_coset_evals[point])
...
fn linearization<F: Scalar, PCSType: HomomorphicPolyComElem<Scalar = F>>(wires: &[F],
                                                                         n: usize,
                                                                         selectors: &[PCSType],
                                                                         k: &[F],
  ...
  for i in 0..witness_polys_eval_beta.len() - 1 {
    let bi = witness_polys_eval_beta[i].add(&gamma.mul(perms_eval_beta[i]))
                                       .add(delta);
    b.mul_assign(&bi);
  }
...
#[allow(non_snake_case)]
pub(super) fn linearization_polynomial_opening<PCS: PolyComScheme,
  ...
  let w = cs.eval_selector_multipliers(witness_polys_eval_beta)
            .unwrap(); // safe unwrap
  linearization::<PCS::Field, PCS::Opening>(&w,
                                            params.group.len(),
                                            &params.selectors,
                                            &params.verifier_params.k,
                                            &params.extended_permutations
                                              [cs.n_wires_per_gate() - 1],
                                            Sigma,
...
#[allow(non_snake_case)]
pub(super) fn linearization_commitment<PCS: PolyComScheme,
                                       CS: ConstraintSystem<Field = PCS::Field>>(
  cs: &CS,
  ...
  let w = cs.eval_selector_multipliers(witness_polys_eval_beta)
            .unwrap(); // safe unwrap
  linearization::<PCS::Field, PCS::Commitment>(&w,
                                               params.cs_size,
                                               &params.selectors,
                                               &params.k,
                                               &params.extended_permutations
...
pub(super) fn eval_public_var_poly<PCS: PolyComScheme>(params: &VerifierParams<PCS>,
  ...
  for ((constraint_index, public_value), lagrange_constant) in
    params.public_vars_constraint_indices
          .iter()
          .zip(public_values)
...
fn compute_sigma_scalar_in_L<F: Scalar>(n: usize,
                                        witness_polys_eval_beta: &[&F],
                                        k: &[F],
                                        challenges: &PlonkChallenges<F>)
                                        -> F {
  ...
  for i in 0..n_wires_per_gate {
    let ai = witness_polys_eval_beta[i].add(&k[i].mul(&gamma_beta))
                                       .add(delta);
    a.mul_assign(&ai);
  }
  ...
  let L1_eval_beta = beta_pow_n.sub(&F::one())
...
#[allow(non_snake_case)]
pub(super) fn derive_Q_eval_beta<PCS: PolyComScheme>(params: &VerifierParams<PCS>,
                                                     proof: &PlonkPf<PCS>,
                                                     challenges: &PlonkChallenges<PCS::Field>,
  ...
  for i in 0..n_wires_per_gate - 1 {
    let b = proof.witness_polys_eval_beta[i].add(&gamma.mul(&proof.perms_eval_beta[i]))
                                            .add(&delta);
    term1.mul_assign(&b);
  }
...
mod test {
  ...
  fn test_sigma_polynomial() {
    ...
    let witness = [one, three, five, four, two, two, two, six, three, five, seven, ten];
    let mut challenges = PlonkChallenges::<F>::new();
    challenges.insert_gamma_delta(one, zero).unwrap();
    let Q = Sigma_polynomial::<KZGCommitmentSchemeBLS, PlonkConstraintSystem<F>>(&cs,
                                                                                 &params,
                                                                                 &witness[..],
                                                                                 &challenges);
