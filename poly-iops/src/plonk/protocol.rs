...
pub mod prover {
  ...
  pub type PlonkPf<PCS> = PlonkProof<<PCS as PolyComScheme>::Commitment,
  ...
  /// ```
  pub fn prover<R: CryptoRng + RngCore,
                  PCS: PolyComScheme,
    ...
    let online_values: Vec<PCS::Field> = cs.public_vars_witness_indices()
                                           .iter()
    ...
    for i in 0..n_wires_per_gate {
      let mut f = FpPolynomial::ffti(root,
    ...
    let (C_Sigma, O_Sigma) = pcs.commit(Sigma).map_err(|_| PlonkError::CommitmentError)?;
    ...
    let witness_polys: Vec<FpPolynomial<PCS::Field>> =
      witness_openings.iter()
    ...
    let (C_q_polys, O_q_polys) = split_Q_and_commit(pcs, &Q, n_wires_per_gate, n_constraints + 2)?;
    ...
    let witness_polys_eval_beta: Vec<PCS::Field> =
      witness_openings.iter()
                      .map(|open| pcs.eval_opening(open, &beta))
                      .collect();
    let perms_eval_beta: Vec<PCS::Field> = params.extended_permutations
                                                 .iter()
    ...
    let O_L = linearization_polynomial_opening::<PCS, CS>(cs,
                                                          params,
                                                          &O_Sigma,
    ...
    // 8. batch eval proofs
    let mut openings: Vec<&PCS::Opening> = witness_openings.iter()
                                                           .chain(params.extended_permutations
                                                                        .iter()
                                                                        .take(cs.n_wires_per_gate()
                                                                              - 1))
                                                           .collect();
    ...
    points.push(g_beta);
    let (_, batch_eval_proof) = pcs.batch_prove_eval(transcript,
                                                     &openings[..],
                                                     &points[..],
                                                     n_constraints + 2,
                                                     None)
  ...
  pub fn verifier<PCS: PolyComScheme, CS: ConstraintSystem<Field = PCS::Field>>(
    ...
    challenges.insert_beta(beta).unwrap();
    for eval_beta in proof.witness_polys_eval_beta
                          .iter()
    ...
    let C_L = linearization_commitment::<PCS, CS>(cs,
                                                  cs_params,
                                                  &proof.C_Sigma,
                                                  &witness_polys_eval_beta_as_ref[..],
                                                  &perms_eval_beta_as_ref[..],
    ...
    let mut commitments: Vec<&PCS::Commitment> = proof.C_witness_polys
                                                      .iter()
                                                      .chain(cs_params.extended_permutations
                                                                      .iter()
                                                                      .take(cs.n_wires_per_gate()
                                                                            - 1))
                                                      .collect();
    ...
    points.push(g_beta);
    let mut values: Vec<PCS::Field> = proof.witness_polys_eval_beta
                                           .iter()
                                           .chain(proof.perms_eval_beta.iter())
                                           .cloned()
                                           .collect();
    ...
    pcs.batch_verify_eval(transcript,
                          &commitments[..],
                          cs_params.cs_size + 2,
                          &points[..],
                          &values[..],
