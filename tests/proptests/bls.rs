...
proptest! {
    ...
    fn simple_signatures(message in "\\PC*") {
      ...
      let (sk, pk) = bls_gen_keys::<_, Bls12381>(&mut prng);
      let signature = bls_sign::<Bls12381, _>(&sk, &message);
    ...
    fn aggregated_signatures(message in  "\\PC*"){
      ...
      let mut prng = rand_chacha::ChaChaRng::from_seed([1u8; 32]);
      let (sk1, pk1) = bls_gen_keys::<_, Bls12381>(&mut prng);
      let (sk2, pk2) = bls_gen_keys::<_, Bls12381>(&mut prng);
      let (sk3, pk3) = bls_gen_keys::<_, Bls12381>(&mut prng);
      ...
      let agg_signature =
        bls_aggregate::<Bls12381>(&keys, &[&signature1, &signature2, &signature3]);
  ...
  fn batching(message1 in  "\\PC*", message2 in  "\\PC*", message3 in  "\\PC*") {
    ...
    let signature1 = bls_sign::<Bls12381, _>(&sk1, &message1);
    let signature2 = bls_sign::<Bls12381, _>(&sk2, &message2);
    let signature3 = bls_sign::<Bls12381, _>(&sk3, &message3);
