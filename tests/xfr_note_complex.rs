...
pub(crate) mod xfr_note_complex_variable_size {
  ...
  use zei::xfr::lib::{gen_xfr_body, verify_xfr_body, XfrNotePolicies};
  ...
  fn check_xfr_body(n: usize) {
    ...
    let (ar_ins, output_asset_records) =
      prepare_inputs_and_outputs_with_policies_single_asset(sender_key_pairs_ref.as_slice(),
                                               user_ac_sks,
                                               credentials,
                                               ac_commitment_keys,
    ...
    let xfr_body = gen_xfr_body(&mut prng,
                                ar_ins.as_slice(),
