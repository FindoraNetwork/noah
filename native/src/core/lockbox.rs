...
pub fn lock(publickey: &PublicKey, message: &[u8]) -> Lockbox {
    ...
    //g^(x*r),g^r
    let (enc_point, bigr) = derive_key(publickey);
    ...
    let out = secretbox_seal(&slice_to_fixed32(&enc_key), message);
...
pub fn unlock(secretkey: &SecretKey, lockedbox: &Lockbox) -> Vec<u8> {
    //g^x = pk. x is secret key, g^R == rand 
    
    //REDERIVED_KEY == (g^R)^x
    let dec_point = lockedbox.rand.decompress().unwrap() * decode_scalar(&secretkey.as_bytes());
    ...
    let out = secretbox_open(&slice_to_fixed32(&dec_key), &lockedbox.data).unwrap();
