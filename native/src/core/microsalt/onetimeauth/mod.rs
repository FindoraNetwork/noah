//! Secret-key One-time authentication
//! # Selected primitive
//! `authenticate()` is `crypto_onetimeauth_poly1305`, an authenticator specified
//! in [Cryptography in `NaCl`](http://nacl.cr.yp.to/valid.html), Section 9. This
//! authenticator is proven to meet the standard notion of unforgeability after a
//! single message.

pub mod poly1305;

pub fn authenticate(message: &[u8], key: &poly1305::OnetimeauthKey) -> [u8;16] {
    let mut result = [0u8;16];
    poly1305::onetimeauth(&mut result, &message, &key);

    result
}

pub fn verify(hash: &poly1305::OnetimeauthHash, message: &[u8], key: &poly1305::OnetimeauthKey) -> bool {
    poly1305::onetimeauth_verify(&hash, &message, &key)
}

#[test]
fn test_poly1305() {
        // corresponding to tests/onetimeauth.c, tests/onetimeauth2.c,
        // tests/onetimeauth5.cpp and tests/onetimeauth6.cpp from NaCl
        
        let key = [0xee, 0xa6, 0xa7, 0x25, 0x1c, 0x1e, 0x72, 0x91, 0x6d, 0x11, 0xc2, 0xcb,
                       0x21, 0x4d, 0x3c, 0x25, 0x25, 0x39, 0x12, 0x1d, 0x8e, 0x23, 0x4e, 0x65,
                       0x2d, 0x65, 0x1f, 0xa4, 0xc8, 0xcf, 0xf8, 0x80];

        let c = [];

        let a_expected = [0xf3, 0xff, 0xc7, 0x70, 0x3f, 0x94, 0x00, 0xe5, 0x2a, 0x7d, 0xfb, 0x4b, 0x3d, 0x33, 0x05, 0xd9];

        let a = authenticate(&c, &key);
        assert!(a == a_expected);
        assert!(verify(&a, &c, &key));
}